...
pub fn main() !void {
    ...
    const r2 = 2;
    const k2 = 5;
    const k1: u32 = 60 * k2 * 3 / (8 * (r1 + r2)); // 60 == screen_width
    ...
    while (true) {
        ...
        var output: [60][60]u8 = undefined;
        for (&output) |*row| {
            row.* = [_]u8{' '} ** 60;
        }
        const zbuffer = std.mem.zeroes([60][60]u8);
        ...
        while (theta < end) : (theta += theta_spacing) {
            ...
            while (phi < 2 * std.math.pi) : (phi += phi_spacing) {
                ...
                const xp: u64 = @intFromFloat(60 / 2 + k1 * ooz * x);
                const yp: u64 = @intFromFloat(60 / 2 - k1 * ooz * y);
